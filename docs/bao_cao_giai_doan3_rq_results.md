...
**PhysX FEM** (`isaac-lab-fem`, DeformableObject): gel block deformable (lưới tet co-rotational, E=1e5, ν=0.45) + indentor cầu rigid; pin node đáy; đọc `nodal_pos_w` → marker grid. **Normal-only** (`isaac_extract_groundtruth.py`) **+ SHEAR** (`isaac_extract_shear.py`, đã phá deadlock — xem §3c).

## 2. Dataset

- **Hertz–Mindlin** `data/phase3_gt/`: train 16k, val 2k, test_id 2k, test_slip 1k, + 8 OOD (radius/depth/material/friction/geometry/resolution). Marker 32×32.
- **FEM normal** `data/phase3_gt_fem/`: 40 frame normal-only, marker 24×24, solve-time logged.
- **FEM shear** `data/fem_gt_shear.npz`: 40 frame có kéo tiếp tuyến, drive-ratio g 0.04–1.28 (stick/partial/full), marker 24×24 (xem §3c).
...
### Cách phá deadlock (`scripts/isaac_extract_shear.py`)
...
Dữ liệu: `data/fem_gt_shear.npz` (40 frame, g 0.04–1.28). Validate: `scripts/validate_shear.py`.
...
→ **Toàn bộ RQ1–RQ3 dưới đây chạy trong framing field→field** (script `phase3_field2field_full.py`, 16k train / 40 epoch, mirror đúng splits của param→field). Framing param→field cũ chuyển xuống **§4b ablation** làm chứng cứ phản chứng.
...
## 9. Tài sản (scripts/ + runs/)
`gt_hertz_mindlin.py`, `phase3_data_gen.py`, `validate_gt.py`, `phase3_train.py` (param→field), `phase3_eval_rq.py`, **`phase3_field2field_full.py` (HEADLINE field→field, RQ1–RQ3)**, `phase3_field2field.py` (PoC), `isaac_extract_groundtruth.py` (FEM normal), **`isaac_extract_shear.py` (FEM shear, phá deadlock)**, `validate_shear.py`, `Dockerfile.fem`, `setup_isaac.sh` · `runs/phase3_f2f_full/results.json` + `fidelity_speed.png` (headline) · `runs/phase3/` (param→field ablation) · `data/phase3_gt_fem/fem_gt.npz` (normal) · `data/fem_gt_shear.npz` (shear).
