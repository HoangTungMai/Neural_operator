...
per-point MLP by $7.35\times$ and the analytic and learned VBTS baselines by
$4.1$--$11.2\times$ on the same IPC ground truth.
(ii)~\emph{Differentiability buys
sample-efficient control}: learning a goal-conditioned policy by backpropagation
through the frozen operator reaches the target with $77\times$ fewer model queries,
runs $23.8\times$ faster, and reaches lower final loss than a gradient-free
...
$4.1$--$11.2\times$, all measured on the \emph{same} IPC ground truth
...
tactile target with $77\times$ fewer target-reaching model queries and
$23.8\times$ less wall-clock than evolution strategies, while also reaching lower final loss
...
different sensor outputs). The operator attains $\relL=0.074$ and a
$1.6^\circ$ tangential direction error, beating every classical and analytic
baseline by $4.1$--$11.2\times$. Two readings stand out. First, first-principles
...
closest at $0.085$ ($1.15\times$ behind FNO), followed by DeepONet at
...
Per regime, the operator remains accurate from normal ($0.069$) through full slip
($0.093$), with stick and partial slip at $0.070$ and $0.069$, respectively.
The multitask slip head reaches macro-F1 $0.937$ (binary slip-F1 $0.975$).
...
TACTO-style (kinematic) & 0.656 & 69.6 & 3 & 8.85 \\
Cattaneo--Mindlin (analytic) & 0.830 & 75.2 & 6 & 11.19 \\
Taxim/FOTS-style (linear) & 0.302 & 9.3 & 14.4k & 4.07 \\
MLP per-point (local) & 0.545 & 41.9 & 134k & 7.35 \\
\midrule
\multicolumn{5}{@{}l}{\textit{Modern dense / operator nets}}\\
DeepONet & 0.115 & 3.2 & 1.05M & 1.55 \\
Galerkin Transformer & 0.151 & 4.1 & 0.30M & 2.04 \\
U-Net & 0.085 & 2.2 & 0.47M & 1.15 \\
\textbf{FNO (ours)} & \textbf{0.074} & \textbf{1.6} & 2.67M & --- \\
\bottomrule
\end{tabular}
\end{table}
...
operator by $2.60\times$ for radius, $1.95\times$ for friction, and $1.36\times$
...
The operator runs at $\sim$$7920$ frames/s ($\sim$$0.13$\,ms latency) versus
$0.192$\,frames/s for a fair single IPC/GIPC solve---a
$\sim$$41{,}253\times$ speedup over IPC (Fig.~\ref{fig:fidspeed}). The production
...
\caption{Fidelity versus speed. FNO and its multitask variant keep low field
...
a final loss of $1.99\times10^{-10}$ against ES's $2.24\times10^{-10}$, while using
$\mathbf{300}$ forward evaluations against ES's $\mathbf{19{,}200}$ ($64\times$
smaller evaluation budget) and finishing in $\mathbf{13.13}$\,s versus
$\mathbf{312.05}$\,s ($23.8\times$ faster). It first reaches the target after
$81$ queries versus $6251$ for ES ($77\times$ fewer). Both sit above a
per-instance oracle ($9.64\times10^{-12}$), the expected price of a single
...
\midrule
autograd (ours) & $1.99\times10^{-10}$ & \textbf{300} & \textbf{13.13} \\
ES (pop 32) & $2.24\times10^{-10}$ & 19{,}200 & 312.05 \\
\midrule
per-instance oracle & $9.64\times10^{-12}$ & --- & --- \\
\bottomrule
\end{tabular}
\end{table}
...
target $77\times$ earlier in query count.}
\label{fig:control}
\end{figure}
...
$12.6\%$ mean magnitude error and $3.4^\circ$ mean direction error; both are
hardest in normal frames because the applied shear magnitude is near zero.
...
current noisy image-reward environment has $7.31\%$ relative error and is therefore
used only as a gradient-flow sanity check, not as formal gradient verification or
evidence for multi-step contact dynamics.
