...
surface marker-displacement field---augmented with a lightweight local
refinement head (LR-FNO, an adaptation of the U-FNO family)---and use it as a
differentiable,
...
$5.0$--$13.8\times$, all measured on the \emph{same} IPC ground truth
(Sec.~\ref{sec:exp-accuracy}). Adding a lightweight local refinement to the
spectral trunk (LR-FNO, adapting the U-FNO family~\cite{wen2022ufno}) improves
the trunk by a further $24\%$ on every one of five seeds (paired $t{=}4.7$)
at $+3.3\%$ parameters.
...
Galerkin Transformer~\cite{cao2021galerkin} extend this family, and
global--local hybrids such as U-FNO~\cite{wen2022ufno} augment Fourier layers
with local convolutional paths. We bring operator
learning to tactile contact, adapt the global--local family to it, and,
crucially, exploit its non-locality and
...
\caption{The differentiable tactile pipeline. The FNO surrogate is trained once
...
($\relL=0.545$, $42^\circ$ direction error), whereas the FNO holds. The operator
framing is therefore not a modelling convenience but the source of the result.

\textbf{Locally-refined variant (main model).} Our reported surrogate augments
the spectral trunk with a lightweight local path: a two-layer convolutional stem
added to the lifting layer, and a small refinement head that reads the trunk
output, the local features, and the raw input. This adapts the global--local
U-FNO family~\cite{wen2022ufno} to tactile fields---we adapt and evaluate, not
propose, the architecture. The variant adds only $3.3\%$ parameters and is
selected by a five-seed paired benchmark (Sec.~\ref{sec:exp-accuracy}), not by
single-seed peak accuracy; we refer to it as \emph{LR-FNO} and keep the plain
FNO trunk as an ablation.
...
IPC/GIPC ground truth, FNO prediction, image residual, and residual marker flow.
