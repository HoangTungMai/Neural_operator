...
from novbts.research.groundtruth.hertz_mindlin import MODE_NAMES, hertz_mindlin_field
...
def save_split(path, params, coords):
    # Preserve the released analytic corpus bit-for-bit.  The modern physical
    # amplitude is reserved for the IPC evaluation baseline, not synthetic GT.
    disp, mode = hertz_mindlin_field(params, coords, legacy_amplitude=True)
    np.savez_compressed(
        path,
