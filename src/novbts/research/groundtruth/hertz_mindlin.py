...
def mindlin_tangential_shift(P, a, E, mu, g):
    """
    Rigid tangential displacement of the stick zone (Mindlin):
      delta_t = (3 mu P (2-nu)) / (16 G a) [1 - (1 - Q/uP)^(2/3)]

    ``g`` is Q/(mu P), clipped at one for full sliding.  This returns metres,
    not a fitted or normalised shape factor.  See Johnson (1985), §7.2.
    """
    G = E / (2.0 * (1.0 + NU))
    gp = np.clip(g, 0.0, 1.0)
    return (
        3.0 * mu * P * (2.0 - NU)
        / (16.0 * G * np.clip(a, 1e-6, None))
        * (1.0 - np.power(1.0 - gp, 2.0 / 3.0))
    )


def bonded_layer_tangential_compliance_factor(
    a: np.ndarray,
    *,
    thickness_m: float = LAYER_THICKNESS_M,
) -> np.ndarray:
    """Return ``delta_layer / delta_halfspace`` for a bonded elastic layer.

    The factor is the parameter-free, two-limit stiffness closure

    ``delta_layer / delta_halfspace = 1 / (1 + pi*(2-nu)/8 * a/h)``.

    It follows by writing the rigid circular-contact stiffness as the
    half-space stiffness plus its bonded thin-layer limit.  For a rigid
    circular contact, ``k_T,HS = 8 G a / (2-nu)`` and, for a thin layer bonded
    to a rigid substrate, ``k_T,thin = pi G a^2 / h``.  Thus their ratio is
    ``k_T,thin/k_T,HS = pi*(2-nu)/8 * a/h``.  This positive lowest-order
    composite is not fit to IPC data: it preserves the exact half-space limit
    and the exact leading thin-layer stiffness limit.

    Source for the two limiting tangential stiffnesses: H. Burger, F.
    Forsbach & V. L. Popov, "Boundary Element Method for Tangential Contact
    of a Coated Elastic Half-Space," *Machines* 11, 694 (2023),
    doi:10.3390/machines11070694, §3.1, Eqs. (61) and (65).  Their finite
    thickness BEM results also show the monotone stiffening between these
    limits.  This is an amplitude/compliance correction only; it is not a
    replacement for the finite-layer partial-slip traction solution.
    """
    if not np.isfinite(thickness_m) or thickness_m <= 0.0:
        raise ValueError(f"thickness_m must be finite and positive, got {thickness_m!r}")
    x = np.asarray(a, dtype=np.float64) / thickness_m
    stiffness_ratio = 1.0 + (math.pi * (2.0 - NU) / 8.0) * x
    return 1.0 / stiffness_ratio


def _legacy_mindlin_tangential_shape(a, E, g):
    """Pre-2026 non-dimensional amplitude helper, retained only for data replay."""
    G = E / (2.0 * (1.0 + NU))
    gp = np.clip(g, 0.0, 1.0)
...
def bonded_layer_contact_radius(
    hertz_a: np.ndarray,
    geom: np.ndarray,
    *,
    thickness_m: float = LAYER_THICKNESS_M,
) -> np.ndarray:
    """Contact radius for a sphere on a bonded, near-incompressible thin layer.

    For a prescribed Hertz load P_H, solve ``P_H = P_H(a) K(a/h)`` with

    ``K(x) = 1 + 1.133x + 1.283x² + 0.769x³ + 0.0975x⁴``.

    This is the bonded, incompressible spherical-indenter correction of
    Dimitriadis et al., *Biophysical Journal* 82 (2002), Eq. 10,
    doi:10.1016/S0006-3495(02)75620-8.  The gel has nu=0.45, so this is a
    near-incompressible approximation, not a full finite-layer Green function.
    Flat punches are outside this spherical formula and are left unchanged.
    """
    if not np.isfinite(thickness_m) or thickness_m <= 0.0:
        raise ValueError(f"thickness_m must be finite and positive, got {thickness_m!r}")
    hertz_a = np.asarray(hertz_a, dtype=np.float64)
    geom = np.asarray(geom)
    lo = np.zeros_like(hertz_a)
    hi = hertz_a.copy()
    # K(x) >= 1 and monotonic for x >= 0, therefore the physical root lies in
    # (0, a_H].  Bisection is robust and fully vectorised over frames.
    for _ in range(64):
        mid = 0.5 * (lo + hi)
        x = mid / thickness_m
        k = 1.0 + x * (1.133 + x * (1.283 + x * (0.769 + x * 0.0975)))
        too_large = (mid / np.clip(hertz_a, 1e-12, None)) ** 3 * k > 1.0
        hi = np.where(too_large, mid, hi)
        lo = np.where(too_large, lo, mid)
    corrected = 0.5 * (lo + hi)
    return np.where(geom > 0.5, hertz_a, corrected)
...
def hertz_mindlin_field(
    params: np.ndarray,
    coords: np.ndarray,
    *,
    shear_scale: float = 1.0,
    legacy_amplitude: bool = False,
):
    """
    params : [N, 9]   coords : [M, 2]   ->   disp [N, M, 3], mode [N]
    Vectorised over frames and markers.

    ``shear_scale`` converts the caller's stored shear units into the
    dimensionless Cattaneo--Mindlin drive ratio::

        g = |shear| / (mu * shear_scale)

    The analytic generator stores ``|shear| = mu*g`` and therefore keeps the
    default ``shear_scale=1``.  The IPC production sweep stores lateral travel
    in metres as ``|shear| = mu*g*1e-3`` and must pass ``shear_scale=1e-3``.

    ``legacy_amplitude=True`` preserves the pre-fairness heuristic in-plane
    amplitudes exactly for the archived analytic dataset generator.  It must
    never be used for the physics baseline evaluated against IPC ground truth.
    """
    if not np.isfinite(shear_scale) or shear_scale <= 0.0:
        raise ValueError(f"shear_scale must be finite and positive, got {shear_scale!r}")
    ...
    g = shear_mag / (np.clip(mu, 1e-6, None) * shear_scale)
    c = mindlin_stick_radius(a_eff, g)

    if legacy_amplitude:
        # Kept verbatim for data_gen.py, whose released analytic files are a
        # reproducible historical artifact rather than this IPC physics baseline.
        ...
        delta_shape = _legacy_mindlin_tangential_shape(a_eff, E, g)
        A = (shear_mag * np.clip(a_eff, 1e-6, None)) * 0.6
        A = A * (0.5 + 0.5 * np.clip(delta_shape / (np.median(delta_shape) + 1e-9), 0.0, 2.0))
    else:
        # No radial term: a closed-form half-space u_r is not implemented here,
        # so retaining the former Gaussian "radial push" would be fabrication.
        ux_r = np.zeros_like(r)
        uy_r = np.zeros_like(r)
        A = mindlin_tangential_shift(P, a_eff, E, mu, g)
...
def hertz_bonded_layer_field(
    ...
    *,
    ...
    """Hertz/Cattaneo--Mindlin field with bonded-layer normal and amplitude corrections.

    The contact radius is corrected by :func:`bonded_layer_contact_radius` for
    the 3-mm, rigidly bonded gel.  The Hertz-shaped normal field is rescaled so
    its centre indentation is ``a_layer²/R`` (the corrected force-indentation
    relation).  The Mindlin tangential shift is multiplied by
    :func:`bonded_layer_tangential_compliance_factor`, an asymptotically matched
    bonded-layer stiffness correction from the circular-contact limits reported
    by Burger, Forsbach & Popov (2023), doi:10.3390/machines11070694.

    No finite-thickness Cattaneo--Mindlin stick-radius law is used here: the
    available correction is for complete-stick tangential compliance, whereas
    this lightweight baseline does not solve the finite-layer partial-slip
    traction integral equation.  Consequently ``c/a=(1-g)^(1/3)`` and the
    existing full-slip profile are deliberately retained rather than fitted to
    IPC fields.  The result is a corrected amplitude, not a complete
    finite-layer tangential displacement Green function.
    """
    if not np.isfinite(shear_scale) or shear_scale <= 0.0:
        raise ValueError(f"shear_scale must be finite and positive, got {shear_scale!r}")
    params = np.asarray(params, dtype=np.float64)
    ...
    dy = coords[None, :, 1] - y0[:, None]
    r = np.sqrt(dx ** 2 + dy ** 2 + 1e-12)
    a, P, _, C = hertz_scalars(depth, radius, E)
    a_eff = bonded_layer_contact_radius(a, geom, thickness_m=thickness_m)
    p0 = 3.0 * P / (2.0 * math.pi * np.clip(a_eff ** 2, 1e-12, None))
    uz = hertz_uz_field(r, a_eff[:, None], p0[:, None], C[:, None])
    # The pressure-normalised half-space field above has centre displacement
    # 3CP/(4a_eff).  At the same load it must instead equal a_eff²/R from the
    # bonded-layer force law, hence this factor (a_eff/a_H)^3.
    normal_scale = np.where(geom > 0.5, 1.0, (a_eff / np.clip(a, 1e-12, None)) ** 3)
    uz = uz * normal_scale[:, None]
    shear_mag = np.sqrt(sx ** 2 + sy ** 2 + 1e-12)
    g = shear_mag / (np.clip(mu, 1e-6, None) * shear_scale)
    # Keep the half-space Cattaneo--Mindlin stick-radius law: a finite-layer
    # partial-slip solution is not justified by the complete-stick compliance
    # correction above (see the function docstring).
    c = mindlin_stick_radius(a_eff, g)
    A = mindlin_tangential_shift(P, a_eff, E, mu, g)
    A = A * bonded_layer_tangential_compliance_factor(
        a_eff, thickness_m=thickness_m
    )
    prof = tangential_profile(r, a_eff[:, None], c[:, None], g)
    ux = A[:, None] * prof * (sx / shear_mag)[:, None]
    uy = A[:, None] * prof * (sy / shear_mag)[:, None]
    disp = np.stack([ux, uy, uz], axis=-1).astype(np.float32)
    mode = np.full(params.shape[0], 1, dtype=np.int32)
    mode[g < G_STICK] = 0
    mode[(g >= G_PARTIAL) & (g < G_FULL)] = 2
    mode[g >= G_FULL] = 3
    return disp, mode
