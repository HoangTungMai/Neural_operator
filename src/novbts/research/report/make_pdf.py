...
def build():
    ...
    body(pdf, "Toàn bộ RQ ở trên đứng trên GT analytic (lý tưởng hoá). Gate 3 yêu cầu làm lại trên VẬT LÝ THẬT. Ta train+đo lại trên tập FEM swept 2000 frame (res-24, quét R/μ/E) qua novbts.research.fno.fem_benchmark → runs/phase3_fem/benchmark.json.")
    ...
    body(pdf, "Số liệu liên-paper KHÔNG so trực tiếp được (cảm biến khác, đầu ra ảnh RGB vs trường marker của ta). Nên ta cài lại LÕI mô hình chuyển động marker của từng phương pháp rồi fit + đo trên CHÍNH GT FEM (cùng data/split/metric, novbts.research.fno.vbts_baselines → runs/phase3_fem/vbts_baselines.json).")
