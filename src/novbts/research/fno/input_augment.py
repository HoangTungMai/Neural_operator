...
the normal channel work. No new data; reuse the existing swept FEM GT.
  ...
  python -m novbts.research.fno.input_augment --data data/fem/shear_fine_swept_normaug.npz
"""
import argparse
import json
...
def boundary_channels(params, coords, side, *, shear_scale=1.0):
    ...
    g = np.sqrt(sx ** 2 + sy ** 2) / (
        np.clip(mu, 1e-6, None) * shear_scale
    )
...
def main():
    ...
    ap.add_argument(
        ...
        type=float,
        default=1e-2,
        help=("stored shear units per dimensionless drive ratio; the default PhysX "
              "swept dataset uses |shear|=mu*g*1e-2"),
    )
    args = ap.parse_args()
    ...
    disp_a, analytic_mode = hertz_mindlin_field(
        D["params"], coords, shear_scale=args.analytic_shear_scale
    )
    if not np.array_equal(analytic_mode, D["mode"].cpu().numpy()):
        n_bad = int(np.count_nonzero(analytic_mode != D["mode"].cpu().numpy()))
        raise RuntimeError(
            f"analytic drive-ratio labels disagree with dataset mode for {n_bad}/{N} frames; "
    ...
    bc = torch.tensor(boundary_channels(
        D["params"], coords, side, shear_scale=args.analytic_shear_scale
    )).to(DEV)                                                               # [N,2,H,W]
