...
Usage:
  python -m novbts.research.fno.vbts_baselines --data data/uipc/shear_res24_avg_swept_REALISTIC_BC.npz
"""
import argparse
import json
import os
...
from novbts.research.groundtruth.hertz_mindlin import (
    MODE_NAMES, LAYER_THICKNESS_M, hertz_mindlin_field, hertz_bonded_layer_field,
)
...
def main():
    ...
    ap.add_argument(
        "--analytic-only",
        action="store_true",
        help=("evaluate only the corrected Cattaneo--Mindlin row without retraining "
              "the learned bake-off models"),
    )
    ap.add_argument(
        "--analytic-shear-scale",
        type=float,
        default=1e-3,
        help=("stored shear units per dimensionless drive ratio; corrected-BC IPC "
              "uses |shear|=mu*g*1e-3 metres"),
    )
    ap.add_argument(
        "--layer-thickness-m", type=float, default=LAYER_THICKNESS_M,
        help="bonded-gel thickness for the Dimitriadis thin-layer correction (metres)",
    )
    args = ap.parse_args()
    from novbts.research.fno.hybrid_fno import LocalRefinedFNO
    ...
    if not args.analytic_only:
        for key, ctor, is_mlp, desc in specs + adv_specs:
            ...
            print(f"[{key:18s}] trained {secs:5.0f}s  params={r['params']:>9d}  "
                  f"overall={r['relative_l2']['overall']:.3f}  "
                  f"tang_dir={r['tangential_dir_error_deg']:5.1f}°")

    # ---- first-principles physics baselines, per-channel affine-calibrated ----
    # This isolates field structure from calibration and gives both textbook Hertz
    # and the 3-mm bonded-layer correction the same opportunity against IPC GT.
    ...
    def evaluate_analytic(key, field_fn, desc, **field_kwargs):
        disp_a, analytic_mode = field_fn(params_np, coords_np, **field_kwargs)
        if not np.array_equal(analytic_mode, D["mode"].cpu().numpy()):
            n_bad = int(np.count_nonzero(analytic_mode != D["mode"].cpu().numpy()))
            raise RuntimeError(
                f"{key} drive-ratio labels disagree with dataset mode for {n_bad}/{N} frames; "
                f"check --analytic-shear-scale={args.analytic_shear_scale:g}"
            )
        ...
        for c in range(3):
            ...
            # SI-valued displacements are O(1e-5).  Solve after centring and
            # scaling so float32 rank detection cannot discard the field column.
            # Convert back to the documented physical affine scale and bias.
            a_mean, a_std = A.mean(), A.std().clamp_min(1e-12)
            M = torch.stack([(A - a_mean) / a_std, torch.ones_like(A)], 1)
            wn = torch.linalg.lstsq(M, Y.unsqueeze(1)).solution.squeeze(1)
            scale = wn[0] / a_std
            bias = wn[1] - scale * a_mean
            affine_scale_bias.append([float(scale.item()), float(bias.item())])
            pred_a[:, c] = scale * af[te, c] + bias
        import time as _t
        _ = field_fn(params_np[te_np], coords_np, **field_kwargs)  # warm
        t0 = _t.perf_counter()
        for _ in range(5):
            field_fn(params_np[te_np], coords_np, **field_kwargs)
        fps = (5 * len(te_np)) / (_t.perf_counter() - t0)
        results[key] = {
            "relative_l2": rel_l2_per_mode(pred_a, out[te], mode[te]),
            "tangential_dir_error_deg": tangential_dir_error(pred_a, out[te], mode[te]),
            "throughput_fps": fps, "params": 6,
            "shear_scale": args.analytic_shear_scale,
            "mode_counts": np.bincount(analytic_mode, minlength=4).tolist(),
            "affine_scale_bias": affine_scale_bias, "desc": desc,
        }
        r = results[key]
        print(f"[{key:20s}] analytic (calibrated)  overall={r['relative_l2']['overall']:.3f}  "
              f"tang_dir={r['tangential_dir_error_deg']:5.1f}°")

    evaluate_analytic(
        "cattaneo_mindlin", hertz_mindlin_field,
        "first-principles Hertz + Cattaneo-Mindlin; physical Mindlin displacement, no invented radial push; per-channel affine-calibrated to FEM",
        shear_scale=args.analytic_shear_scale,
    )
    evaluate_analytic(
        "hertz_bonded_layer", hertz_bonded_layer_field,
        "Hertz/Cattaneo-Mindlin with the Dimitriadis et al. bonded near-incompressible 3-mm layer stiffness correction; per-channel affine-calibrated to FEM",
        shear_scale=args.analytic_shear_scale, thickness_m=args.layer_thickness_m,
    )
    ...
    if args.analytic_only:
        summary = {
            ...
            "test_frames": nt,
            "models": results,
            "note": ("Fair analytic-only rerun; explicit shear_scale converts stored IPC "
                     "travel to the dimensionless drive ratio; all rows use train-only "
                     "per-channel affine calibration."),
        }
        out_path = phase_dir / "analytic_baselines_fair.json"
    ...
    summary = {
        ...
        "models": results,
        "groups": {
            "vbts_sim_marker_models": ["tacto_kinematic", "cattaneo_mindlin", "hertz_bonded_layer",
